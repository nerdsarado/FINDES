...
WORKDIR /src
COPY ["FINDES/FINDES.csproj", "FINDES/"]
RUN dotnet restore "./FINDES/FINDES.csproj"
COPY . .
WORKDIR "/src/FINDES"
RUN dotnet build "./FINDES.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Esta fase é usada para publicar o projeto de serviço a ser copiado para a fase final
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./FINDES.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false

# Esta fase é usada na produção ou quando executada no VS no modo normal (padrão quando não está usando a configuração de Depuração)
FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "FINDES.dll"]